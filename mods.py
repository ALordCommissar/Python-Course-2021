...
def fight(monster1,monster2,suspense = False,printing = True):
    ...
    while True:
        ...
        if printing:
            ...
            monster1.show()
            monster2.show()
        if monster1.spellcaster == True:
            monster2.damage(monster1.magicMissile())
            if not monster2.alive:
                break
        if monster2.spellcaster == True:        
            monster1.damage(monster2.magicMissile())
            if not monster1.alive:
                break
        if monster1.spellcaster == False:
            monster2.damage(monster1.hit())
            if not monster2.alive:
                break
        if monster2.spellcaster == False:
...
zombie = genericMonster(22,Die(6),1,"zombie")
wizard1 = Wizard()
wizard2 = Wizard()
fight(zombie,wizard1,True,True)
wizard1.refresh()
fight(wizard1,wizard2,True,True)
